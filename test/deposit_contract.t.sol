...
contract DepositContractTest is Test {
    ...
    function test_16_deposits(
        bytes32[16] memory pubkey_one,
        bytes16[16] memory pubkey_two,
        bytes32[16] memory _withdrawal_credentials,
        bytes32[16] memory sig_one,
        bytes32[16] memory sig_two,
        bytes32[16] memory sig_three,
        uint32[16] memory amount
    ) public {
        uint256 j;
        for (uint256 i = 0; i < 16; i++) {
            ...
            if (1 ether <= gweiamount) {
                ...
                assertEq(depositContract_sol.get_deposit_root(), depositContract_vyp.get_deposit_root());
                assertEq(
                    keccak256(abi.encodePacked(depositContract_sol.get_deposit_count())),
                    keccak256(abi.encodePacked(depositContract_vyp.get_deposit_count()))
                );
                assertEq(
                    keccak256(abi.encodePacked(depositContract_sol.get_deposit_count())),
                    keccak256(to_little_endian_64(uint64(j)))
                );
            }
        }
    }
    ...
    function testFail_deposit_not_divisible_by_gwei(
        bytes32 pubkey_one,
        bytes16 pubkey_two,
    ...
    function testFail_malformed_node_vyp(
        bytes32 pubkey_one,
        bytes16 pubkey_two,
        bytes32 _withdrawal_credentials,
        bytes32 sig_one,
    ...
    function testFail_malformed_node_sol(
        bytes32 pubkey_one,
        bytes16 pubkey_two,
        bytes32 _withdrawal_credentials,
        bytes32 sig_one,
    ...
    function testFail_malformed_calldata_vyp(
        bytes memory pubkey,
        ...
        if (amount >= 1000000000000000000) {
            if (!(pubkey.length == 48 && withdrawal_credentials.length == 32 && signature.length == 96)) {
                depositContract_vyp.deposit{value: amount}(
                    pubkey,
    ...
    function testFail_malformed_calldata_sol(
        bytes memory pubkey,
        bytes memory withdrawal_credentials,
        bytes memory signature,
        uint64 amount
        ...
        if (amount >= 1000000000000000000) {
            if (!(pubkey.length == 48 && withdrawal_credentials.length == 32 && signature.length == 96)) {
                depositContract_sol.deposit{value: amount}(
                    pubkey,
                    withdrawal_credentials,
                    signature,
                    encode_node(pubkey, withdrawal_credentials, signature, to_little_endian_64(amount / GWEI))
